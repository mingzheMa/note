...
我们将之前的 reactive 方法改名为 createReactive，并接受第二个参数 options，判断如果传入了 options.shallow === false 则通过递归返回一个响应式数据。这样我们就可以通过 options.shallow 配置来实现控制浅响应和深响应了，只需要对 createReactive 进行封装
