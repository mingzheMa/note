...
索引和length是数组中可以修改的属性，我们已经实现了其响应式
...
trigger 中遍历该数组的 key 以及对应的副作用列表，将大于修改后 length（也就是newValue） 的 key 触发更新，这样在执行例子1，就可以触发副作用

之后我们看例子2，当设置数组索引大于等于 length 的时候需要触发 length 的副作用函数，当设置数组索引小于 length 时则不需要触发
