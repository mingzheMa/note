...
<code>onBeforeMount(() => {})</code> 是在 <code>setup</code> 执行时调用，<code>setup</code> 是在组件挂载阶段中设置组件实例时调用，也就是生命周期钩子的注册时机就是组件挂载阶段，在 <code>setup</code> 函数调用期间会将 <code>currentInstance</code> 设置为当前组件实例（执行完毕后会重置），因此 <code>target</code> 参数为空则表示 <code>onBeforeMount(() => {})</code> 并不是在 <code>setup</code> 函数内调用
